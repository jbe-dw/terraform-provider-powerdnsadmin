// Code generated by go-swagger; DO NOT EDIT.

package zones

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"

	"github.com/go-openapi/runtime"
	"github.com/go-openapi/strfmt"
)

// NotifyZoneReader is a Reader for the NotifyZone structure.
type NotifyZoneReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the received o.
func (o *NotifyZoneReader) ReadResponse(response runtime.ClientResponse, consumer runtime.Consumer) (interface{}, error) {
	switch response.Code() {
	case 200:
		result := NewNotifyZoneOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		return nil, runtime.NewAPIError("response status code does not match any response statuses defined for this endpoint in the swagger spec", response, response.Code())
	}
}

// NewNotifyZoneOK creates a NotifyZoneOK with default headers values
func NewNotifyZoneOK() *NotifyZoneOK {
	return &NotifyZoneOK{}
}

/*NotifyZoneOK handles this case with default header values.

OK
*/
type NotifyZoneOK struct {
}

func (o *NotifyZoneOK) Error() string {
	return fmt.Sprintf("[PUT /servers/{server_id}/zones/{zone_id}/notify][%d] notifyZoneOK ", 200)
}

func (o *NotifyZoneOK) readResponse(response runtime.ClientResponse, consumer runtime.Consumer, formats strfmt.Registry) error {

	return nil
}
